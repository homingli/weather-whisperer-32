...
- **Component tests** (components/):
  ...
  - `src/components/RainStartBanner.test.tsx` (6): rain-expected / raining-now / no-rain verdict copy, city-scale qualifier, sparkline span label (gated to the sparkline's ≥2-bars condition), live-region content stable across minute ticks (countdown clause excluded), wet windows beyond the 6-hour verdict horizon ignored, renders nothing when no series is usable.
